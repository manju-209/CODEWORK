  ...
  return (
    <section className="relative w-full min-h-screen flex items-center bg-primary overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-28 left-16 w-2 h-2 md:w-3 md:h-3 bg-secondary rounded-full animate-pulse" />
        <div className="absolute top-48 right-20 w-1 h-1 md:w-2 md:h-2 bg-white/60 rounded-full animate-bounce" />
        <div className="absolute bottom-56 left-1/4 w-3 h-3 md:w-4 md:h-4 bg-secondary/80 rounded-full animate-ping" />
        <div className="absolute top-3/4 right-1/3 w-1 h-1 md:w-2 md:h-2 bg-white/40 rounded-full animate-pulse" />
      </div>
      ...
      <div className="relative z-10 w-[80%] mx-auto px-4 text-left">
        ...
        <div className="paragraph-container max-w-3xl">
          <p className="text-lg sm:text-xl text-white/90 leading-relaxed font-light pl-4 border-l-4 border-secondary/50">
            Discover the secrets of integrating{" "}
            <span className="text-secondary font-medium">AI in e-commerce</span> 
            and unlock remarkable success for your business with our expert strategies 
            and actionable insights. From personalized shopping experiences to smart inventory 
            management, learn how AI can transform your retail growth.
          </p>
        </div>
      </div>
